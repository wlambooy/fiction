...
TEST_CASE("SiQAD OR gate", "[is-operational]")
{
    ...
    auto op_params = is_circuit_operational_params<cell<sidb_100_cell_clk_lyt_siqad>>{
        ...
        {},
        is_circuit_operational_params<cell<sidb_100_cell_clk_lyt_siqad>>::simulation_results_mode::KEEP_SIMULATION_RESULTS};

    SECTION(
        "determine if layout is operational, tolerate kinks, assess all input combinations and keep simulation results")
    {
        const auto assessment_results = is_circuit_operational(lat, std::vector<tt>{create_or_tt()}, op_params);
