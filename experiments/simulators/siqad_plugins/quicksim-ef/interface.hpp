...
class siqad_plugin_interface
{
  public:
    ...
    siqad_plugin_interface(const std::string_view& t_in_path, const std::string_view& t_out_path,
                           ...
                           fiction::sidb_simulation_engine engine = fiction::sidb_simulation_engine::QUICKSIM) :
            log_level{log_l},
            in_path{t_in_path},
            out_path{t_out_path},
  ...
  private:
    ...
    void initialize_fiction_layout()
    {
        ...
        try
        {
            ...
            params.ef_params.global_theta = std::stod(sqconn->getParameter("global_theta"));
            params.ef_params.local_theta = std::stod(sqconn->getParameter("local_theta"));
            params.ef_params.stability_error = std::stod(sqconn->getParameter("stability_error"));
            params.ef_params.energy_error = std::stod(sqconn->getParameter("energy_error"));
