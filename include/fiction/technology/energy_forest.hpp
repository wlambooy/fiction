  ...
  public:
                                  ...
                                  const sidb_simulation_parameters&     phys_params)
    {
        ...
        for (uint64_t i = 0; i < ef->root_collection->sidbs.size(); ++i)
        {
            ...
            if (lpe->ets.size() > 1)
            {
                for (const uint64_t j : lpe->to_greatest_lower_bound(ef->phys_params))
                {
                    if (j == i)
                    {
                        continue;
        ...
        for (uint64_t i = 0; i < ef->root_collection->sidbs.size(); ++i)
        {
            ...
            for (uint64_t j = 0; j < local_potential_expressions[i]->ets.size(); ++j)
            {
                for (const uint64_t k : local_potential_expressions[i]->ets[j]->c2->sidbs)
                {
                    if (reference_store.count(k) == 0)
                    {
                        local_potential_expressions[k]->update_set.emplace(local_potential_expressions[i], j);
                    }
                }
            }
        }
    ...
    std::shared_ptr<energy_forest_worker> make_new_worker(const std::vector<sidb_charge_state>& cell_charge)
    {
        ...
        for (uint64_t i = 0; i < ef->primary_worker->unique_local_potential_expressions.size(); ++i)
        {
            w->unique_local_potential_expressions.push_back(
                ef->primary_worker->local_potential_expressions[i]->copy(w->system_energy));
        }
        ...
        for (uint64_t i = 0; i < w->unique_local_potential_expressions.size(); ++i)
        {
            ...
            for (const auto& [lpe, j] : w->unique_local_potential_expressions[i]->update_set)
            {
