...
class charge_distribution_surface<Lyt, ExtPotType, true> : public Lyt
{
  public:
...
template <typename Lyt, local_external_potential_type ExtPotType>
class charge_distribution_surface<Lyt, ExtPotType, false> : public Lyt
{
  public:
    using charge_index_base = std::pair<uint64_t, uint8_t>;

    using maybe_bounded_double =
        std::conditional_t<ExtPotType == local_external_potential_type::SINGLE_VALUED, double, std::array<double, 2>>;

    using local_external_potential_map_t = std::unordered_map<cell<Lyt>, maybe_bounded_double>;
    ...
    struct charge_distribution_storage
    {
      private:
        ...
        using maybe_bounded_local_potential = std::vector<maybe_bounded_double>;
      ...
      public:
         ...
         */
        local_external_potential_map_t local_external_potential_map{};
        /**
         ...
         */
        maybe_bounded_local_potential local_ext_pot;
        /**
         * Local electrostatic potential generated by charged SiDBs and defects.
         */
        local_potential local_int_pot;
        /**
         * Electrostatic potential generated by charged SiDBs and defects that is local to defects.
         */
        std::unordered_map<typename Lyt::cell, double> local_int_pot_at_defect{};
        /**
         * Electrostatic potential generated from external sources that is local to defects.
         */
        std::unordered_map<cell<Lyt>, double> local_ext_pot_at_defect{};  // todo
        /**
         * Stores the electrostatic energy of a given charge distribution (unit: eV).
         */
        maybe_bounded_double system_energy{};
        /**
        ...
        for (uint64_t i = 0; i < strg->sidb_order.size(); ++i)
        {
            if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
            {
                strg->charge_transition_threshold_bounds[i] = {
                    strg->local_ext_pot[i][1] - strg->simulation_parameters.mu_minus +
                        constants::ERROR_MARGIN,  // DB- (UB)
                    strg->local_ext_pot[i][0] - strg->simulation_parameters.mu_plus() -
                        constants::ERROR_MARGIN,  // DB+ (LB)
                    strg->local_ext_pot[i][0] - strg->simulation_parameters.mu_minus -
                        constants::ERROR_MARGIN,  // DB0 (LB)
                    strg->local_ext_pot[i][1] - strg->simulation_parameters.mu_plus() +
                        constants::ERROR_MARGIN,  // DB0 (UB)
                };
            }
            else
            {
    ...
    [[nodiscard]] std::vector<uint64_t> negative_sidb_detection() const noexcept
    {
        ...
        for (uint64_t i = 0; i < strg->sidb_order.size(); ++i)
        {
            ...
            // given in eV).
            if (const double local_pot = *get_local_potential_by_index(i);
    ...
    /**
     * This function calculates the internal local electrostatic potential in Volt for each SiDB position, which
     * includes electrostatic potentials generated by SiDBs and defects (unit: V).
     *
    ...
    /**
     * This function calculates the external local electrostatic potential in Volt for each SiDB and defect position,
     * which includes only external electrostatic potentials (generated by electrodes, etc.) (unit: V).
     */
    void update_local_external_potential() noexcept
    {
        ...
        {
            strg->local_ext_pot =
                std::vector<std::array<double, 2>>(this->num_cells(), std::array<double, 2>{0.0, 0.0});
        }
        else
        {
        ...
        strg->local_ext_pot_at_defect.clear();
        ...
        for (const auto& [c, external_pot] : strg->local_external_potential_map)
        {
            if (strg->defects.find(c) == strg->defects.cend())
            {
                ...
                const auto ix = cell_to_index(c);

                if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
                {
                    strg->local_ext_pot[static_cast<uint64_t>(ix)][0] = external_pot[0];
                    strg->local_ext_pot[static_cast<uint64_t>(ix)][1] = external_pot[1];
                }
                else
                {
            ...
            else
            {
                ...
                if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
                {
                    strg->local_ext_pot_at_defect[c] = external_pot[0];  // todo
                }
                else
                {
    ...
    /**
     * This function calculates the internal local electrostatic potential in Volt for each defect position, which
     * includes electrostatic potentials generated by SiDBs and defects (unit: V).
     */
    void update_local_defect_potential() noexcept
    {
        strg->local_int_pot_at_defect.clear();

        for (const auto& [c1, defect1] : strg->defects)
        {
            for (const auto& [c2, defect2] : strg->defects)
            {
                const auto dist = sidb_nm_distance<Lyt>(*this, c1, c2);
                const auto pot  = chargeless_potential_generated_by_defect_at_given_distance(dist, defect2);

                strg->local_int_pot_at_defect[c1] += pot * static_cast<double>(defect2.charge);
            }
     ...
     * returned (unit: V).
     */
    std::optional<maybe_bounded_double> get_local_potential(const typename Lyt::cell& c) const noexcept
    {
        if (const auto index = cell_to_index(c); index != -1)
        {
            if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
     ...
     */
    [[nodiscard]] std::optional<maybe_bounded_double> get_local_potential_by_index(const uint64_t index) const noexcept
    {
    ...
    /**
     * This function returns the internal local electrostatic potential at a given index position in Volt (unit: V).
     *
     * @param index The index defining the SiDB position.
     * @return Internal local potential at given index position. If there is no SiDB at the given index (which
     * corresponds to a unique cell), `std::nullopt` is returned (unit: V).
     */
    [[nodiscard]] std::optional<double> get_local_internal_potential_by_index(const uint64_t index) const noexcept
    {
        if (index < strg->sidb_order.size())
        {
            return strg->local_int_pot[index];
        }
        return std::nullopt;
    }
    /**
     ...
     * returned (unit: V).
     */
    std::optional<maybe_bounded_double> get_local_external_potential(const typename Lyt::cell& c) const noexcept
    {
        if (const auto index = cell_to_index(c); index != -1)
        {
     ...
     *
     * @param index The index defining the SiDB position.
     * @return External local potential at given index position. If there is no SiDB at the given index (which
     * corresponds to a unique cell), `std::nullopt` is returned (unit: V).
     */
    [[nodiscard]] std::optional<maybe_bounded_double>
    get_local_external_potential_by_index(const uint64_t index) const noexcept
    {
        if (index < strg->sidb_order.size())
        {
            return strg->local_ext_pot[index];
        }
        return std::nullopt;
    }
    /**
    ...
    get_local_potential_caused_by_defects_by_index(const uint64_t index) const noexcept
    {
        if (index < strg->sidb_order.size())
        {
            return strg->local_pot_caused_by_defects[index];
        }
        return std::nullopt;
    }
    /**
     * The function returns the local electrostatic potential at a given defect position in V.
     *
     * @param c The cell defining the defect position.
     * @return Local potential at given cell position. If there is no defect at the given cell, `std::nullopt` is
     * returned (unit: V).
     */
    [[nodiscard]] std::optional<double> get_local_defect_potential(const typename Lyt::cell& c) const noexcept
    {
        if (strg->local_int_pot_at_defect.count(c) && strg->local_ext_pot_at_defect.count(c))
        {
    ...
    void assign_electrostatic_potential_energy_to_zero() noexcept
    {
        if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
        {
            strg->system_energy = std::array<double, 2>{0.0, 0.0};
        }
        else
        {
    ...
    void recompute_electrostatic_potential_energy() noexcept
    {
        ...
        if (strg->engine == sidb_simulation_engine::QUICKSIM)
        {
            ...
            if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
            {
                strg->system_energy = {0.5 * collect, 0.5 * collect};
            }
            else
            {
        ...
        maybe_bounded_double collect_ext{};

        for (uint64_t i = 0; i < strg->sidb_order.size(); ++i)
        {
            if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
            {
                double min_ext_pot_charged =
                    strg->local_ext_pot[i][0] * static_cast<double>(charge_state_to_sign(strg->cell_charge[i]));
                double max_ext_pot_charged =
                    strg->local_ext_pot[i][1] * static_cast<double>(charge_state_to_sign(strg->cell_charge[i]));

                collect_ext[0] += std::min(min_ext_pot_charged, max_ext_pot_charged);
                collect_ext[1] += std::max(min_ext_pot_charged, max_ext_pot_charged);
            }
            else
            {
        ...
        for (const auto& [c, defect] : strg->defects)
        {
            ...
            {
                // todo
                collect_ext[0] += strg->local_ext_pot_at_defect[c] * static_cast<double>(defect.charge);
                collect_ext[1] += strg->local_ext_pot_at_defect[c] * static_cast<double>(defect.charge);
            }
            else
            {
        ...
        if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
        {
            strg->system_energy[0] = collect_ext[0] + 0.5 * collect;
            strg->system_energy[1] = collect_ext[1] + 0.5 * collect;
        }
        else
        {
     ...
     */
    [[nodiscard]] maybe_bounded_double get_electrostatic_potential_energy() const noexcept
    {
     ...
     * otherwise.
     */
    template <bool consider_NONE_charge_states = false, bool debug = false>
        ...
        const charge_distribution_history history_mode            = charge_distribution_history::NEGLECT) noexcept
    {
        ...
        this->validity_check<consider_NONE_charge_states, debug>();
    }
    /**
    ...
    [[nodiscard]] bool is_configuration_stable() const noexcept
    {
            ...
            [this](const uint64_t c1, const uint64_t c2)  // energy change when charge hops between two SiDBs.
        {
            if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
            {
                // // todo: strict or non-strict?
                // return strg->local_ext_pot[c1][0] - strg->local_ext_pot[c2][1] +
                //        (0.5 * (strg->local_int_pot[c1] - strg->local_int_pot[c2] - strg->pot_mat[c1][c2]));
                // non strict version

                // return strg->local_ext_pot[c1][1] - strg->local_ext_pot[c2][0] +
                //        (0.5 * (strg->local_int_pot[c1] - strg->local_int_pot[c2] - strg->pot_mat[c1][c2]));
                return strg->local_ext_pot[c1][1] - strg->local_ext_pot[c2][0] + strg->local_int_pot[c1] -
                       strg->local_int_pot[c2] - strg->pot_mat[c1][c2];
                // todo this is only for special local external potential
            }
            else
            {
        ...
        for (uint64_t i = 0u; i < strg->sidb_order.size(); ++i)
        {
            ...
            for (uint64_t j = 0u; j < strg->sidb_order.size(); j++)
            {
                    ...
                    -constants::ERROR_MARGIN)  // Checks if energetically favored hops exist between two SiDBs.
                {
                    if constexpr (debug)
                    {
                        std::cout << "fault at " << i << " = " << strg->sidb_order[i].x << ',' << strg->sidb_order[i].y
                                  << " with loc pot " << strg->local_int_pot[i] << " and cs "
                                  << charge_configuration_to_string({strg->cell_charge[i]}) << " and ext pot ("
                                  << strg->local_ext_pot[i][0] << "," << strg->local_ext_pot[i][1] << ")" << std::endl;
                        // << strg->local_ext_pot[i] << ")" << std::endl;
                        std::cout << "together with " << j << " = " << strg->sidb_order[j].x << ','
                                  << strg->sidb_order[j].y << " with loc pot " << strg->local_int_pot[j] << " and cs "
                                  << charge_configuration_to_string({strg->cell_charge[j]}) << " and ext pot ("
                                  << strg->local_ext_pot[j][0] << "," << strg->local_ext_pot[j][1] << ")" << std::endl;
                        // << strg->local_ext_pot[j] << ")" << std::endl;
                    }
                    return false;
                }
            }
    ...
    void validity_check() noexcept
    {
        ...
        strg->validity = is_configuration_stable<consider_NONE_charge_states, debug>();
    }
    /**
    ...
    {
        static_assert(ExtPotType == local_external_potential_type::SINGLE_VALUED,
                      "Local external potential must be single-valued.");
    ...
    bool is_three_state_simulation_required() noexcept
    {
        ...
        assert(this->num_negative_sidbs() == this->num_cells() && "All SiDBs have to be negatively charged");
    ...
    [[nodiscard]] double chargeless_potential_at_given_distance(const double distance) const noexcept
    {
        ...
        return (strg->simulation_parameters.k() / (distance * 1E-9) *
                std::exp(-distance / strg->simulation_parameters.lambda_tf) * constants::physical::ELEMENTARY_CHARGE);
    }
    /**
     ...
     * @param defect Defect (including defect specific parameters).
     ...
     *
     * @param external_potential External electrostatic potential store mapping different cells to Volt applied to them.
     */
    void assign_local_external_potential(const local_external_potential_map_t& external_potential) noexcept
    {
        for (const auto& [c, pot] : external_potential)
        {
            if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
            {
                strg->local_external_potential_map[c][0] += external_potential.at(c)[0];
                strg->local_external_potential_map[c][1] += external_potential.at(c)[1];
            }
            else
            {
                strg->local_external_potential_map[c] += external_potential.at(c);
            }
        }
     ...
     */
    local_external_potential_map_t& get_local_external_potentials_reference() noexcept
    {
        return strg->local_external_potential_map;
    }
    /**
    ...
    {
        static_assert(ExtPotType == local_external_potential_type::BOUNDED, "Local external potential must be bounded");
        assert(is_physically_valid() && "The charge distribution must be physically valid");

        const double mu_p = strg->simulation_parameters.mu_plus();

        for (uint64_t i = 0; i < strg->sidb_order.size(); ++i)
        {
            const cell<Lyt>& c = index_to_cell(i);

            switch (strg->cell_charge[i])
            {
                // todo use thresholds?
     ...
     *
     * @return Local electrostatic potential in Volt generated by the defects at each cell.
     */
    [[nodiscard]] std::vector<double> get_local_defect_potentials() const noexcept
    {
     ...
     */
    [[nodiscard]] std::unordered_map<typename Lyt::cell, const sidb_defect> get_defects() const noexcept
    {
        return strg->defects;
    }
    /**
  ...
  private:
     ...
     *
     * @param cs Charge state to assign to all SiDBs.
     * @param configuration Specifies the configuration for charge distribution settings.
     *                      Determines whether only charge locations are considered or if
     *                      both charge locations and electrostatic interactions are included.
     */
     ...
     *  The stored unique index is converted to a charge distribution.
     *
     *  @param charge_index_recomputation_mode Flag that can be set to `IGNORE_LEADING_ZEROES` if the charge index
