...
class charge_distribution_surface<Lyt, ExtPotType, false> : public Lyt
{
  public:
    ...
    using local_external_potential_map_t =
        std::conditional_t<ExtPotType == local_external_potential_type::SINGLE_VALUED,
                           std::unordered_map<cell<Lyt>, double>, std::unordered_map<cell<Lyt>, std::array<double, 2>>>;
    ...
    struct charge_distribution_storage
    {
      private:
        ...
        using bounded_local_potential = std::vector<std::array<double, 2>>;
      ...
      public:
         ...
         */
        local_potential local_ext_pot_at_defect{};
        /**
    ...
    constexpr bool is_local_external_potential_bounded() noexcept
    {
    ...
    void update_local_external_potential() noexcept
    {
        ...
        for (const auto& [c, external_pot] : strg->local_external_potential_map)
        {
            if (const auto ix = cell_to_index(c); ix == -1)
            {
                ...
                {
                    strg->local_ext_pot_at_defect[c] = external_pot[0];
                }
                else
                {
            ...
            else
            {
                ...
                if constexpr (is_local_external_potential_bounded())
                {
                    strg->local_ext_pot[static_cast<uint64_t>(ix)] = external_pot[0];
                }
                else
                {
     ...
     * corresponds to a unique cell), `std::nullopt` is returned (unit: V).
     */
    [[nodiscard]] std::optional<double> get_local_external_potential_by_index(const uint64_t index) const noexcept
    {
        if (index < strg->sidb_order.size())
        {
    ...
    void recompute_electrostatic_potential_energy() noexcept
    {
        ...
        for (const auto& [c, defect] : strg->defects)
        {
            if constexpr (is_local_external_potential_bounded())
            {
                collect_ext += strg->local_ext_pot_at_defect[c].first * static_cast<double>(defect.charge);
            }
            else
            {
    ...
    {
        static_assert(is_local_external_potential_bounded(), "Local external potential must be bounded");
        ...
        for (uint64_t i = 0; i < strg->sidb_order.size(); ++i)
        {
            ...
            switch (strg->cell_charge[i])
            {
                case sidb_charge_state::NEGATIVE:
                    // adjust lower bound
                    strg->local_ext_pot[i][0] += std::min(
                        0, (strg->simulation_parameters.mu_minus - constants::ERROR_MARGIN) - strg->local_pot[i][0]);
                    break;
                case sidb_charge_state::POSITIVE:
                    // adjust upper bound
                    strg->local_ext_pot[i][1] -= std::min(0, strg->local_pot[i][1] - (mu_p + constants::ERROR_MARGIN));
                    break;
                case sidb_charge_state::NEUTRAL:
                    // adjust both upper and lower bound
                    strg->local_ext_pot[i][0] += std::min(0, (mu_p - constants::ERROR_MARGIN) - strg->local_pot[i][0]);
                    strg->local_ext_pot[i][1] -= std::min(
                        0, strg->local_pot[i][1] - (strg->simulation_parameters.mu_minus + constants::ERROR_MARGIN));
                    break;
