...
class is_circuit_operational_impl
{
  public:
    ...
    [[nodiscard]] circuit_operational_assessment<Lyt, ExtPotType> run() noexcept
    {
        ...
        for (auto i = 0u; i < 1 << implemented_circuit.circuit.num_inputs; ++i, ++bii)
        {
            ...
            auto simulation_results = physical_simulation_of_layout(bii, i);
                                              ...
                                              const uint64_t input_pattern) noexcept
    {
        ...
        // const bool print_it = std::round(static_cast<double>(std::rand()) / (RAND_MAX + 1.0) * 10000) == 5000;
        const bool print_it = parameters.print;
        ...
        for (const bdl_wire<Lyt>& wire : implemented_circuit.circuit.bdl_wires)
        {
            for (const bdl_pair<cell<Lyt>>& pair : wire.pairs)
            {
                if (pair.type == sidb_technology::cell_type::INPUT &&
                    implemented_circuit.circuit.super_circuit.skeleton.get_cell_type(pair.upper) !=
                ...
                if (upper_cs != sidb_charge_state::NONE)
                {
                    ...
                    if (print_it)
                    {
                        std::cout << "OK at " << pair.upper.x << "," << pair.upper.y << std::endl;
                    }
                }
                ...
                if (lower_cs != sidb_charge_state::NONE)
                {
                    ...
                    if (print_it)
                    {
                        std::cout << "OK at " << pair.lower.x << "," << pair.lower.y << std::endl;
                    }
                }

                ++successful_bdl_pairs_count;
            }
        }
        ...
        if (print_it)
        {
            ...
            std::cout << "total number of bdl pairs: " << implemented_circuit.circuit.num_bdl_pairs - omit_input_count
                      << std::endl;
  ...
  private:
    ...
    physical_simulation_of_layout(const bdl_input_iterator<Lyt>& bdl_iterator, const uint64_t input_index) noexcept
    {
        ...
        if constexpr (ExtPotType == local_external_potential_type::BOUNDED)
        {
            ...
            for (const uint64_t i :
                 implemented_circuit.circuit.get_consistent_super_circuit_input_indices(input_index).get())
            {
                const charge_distribution_surface<Lyt, local_external_potential_type::BOUNDED>&
                    expected_charge_distribution_for_input =
                        *implemented_circuit.circuit.super_circuit_simulated_bdl_wires_per_input.at(input_index).at(i);
